...
func DownloadHandle(location int, frmts []Ytdl) {
	if location == -1 {
		...
		PrintAvailable(os.Args[2])
		// Ask user what format to use
		format := utils.Printer.Promtln("Please enter the format you'd like to use: ", pringo.None)
		// Download
		Download(os.Args[2], format, os.Args[4])
	} else {
		switch os.Args[3] {
		case "audio":
			Download(os.Args[2], frmts[location].Audio, os.Args[4])
		case "video":
			Download(os.Args[2], frmts[location].Video, os.Args[4])
		case "combined":
			Download(os.Args[2], frmts[location].AudioVideo, os.Args[4])
